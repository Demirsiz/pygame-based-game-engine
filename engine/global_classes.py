...
class Border(pygame.sprite.Sprite):
    def __init__(self,posx,posy,width,height,color,isWallJumpable=True):
...
class Mob(pygame.sprite.Sprite):
    ...
    def checkCollision(self,spriteGroup):
        ...
        for object in listOfCollidingSolidObjects:
            collisionsHorizontalDirection , collisionsVerticalDirection, lastVerticalCollider, lastHorizontalCollider = myFunctions.rectCollisionDirection(self,object)
...
class Wall(pygame.sprite.Sprite):
    def __init__(self,posx,posy,width,height,color="yellow",isWallJumpable=False,texture=None):
        pygame.sprite.Sprite.__init__(self)
...
class Weapon(pygame.sprite.Sprite):
    ...
    def update(self,dt):
        ...
        if(self.isAttacking==True):
            ...
            if(self.owner.facing=="right"):
                self.image=self.texture
                self.rect.topleft=(self.owner.rect.right-5,self.owner.posy+5)
            elif(self.owner.facing=="left"): 
                self.image=pygame.transform.flip(self.texture,True,False)
                self.rect.topright=(self.owner.rect.left+5,self.owner.posy+5)
    ...
    def useAttack(self,owner,other):
            if(self.isReady==True):
                horizontalDistance, verticalDistance, relativeDirection=myFunctions.spriteDistance(owner,other)
                self.isReady=False
...
class SpriteSheet(pygame.sprite.Sprite):
    ...
    def getSprite(self,posx,posy,width,height):
        rawSprite=pygame.Surface((width,height),pygame.SRCALPHA)
        rawSprite.blit(self.spriteSheet,(0,0),(posx,posy,width,height))
